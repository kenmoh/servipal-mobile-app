...
const TransferDetailScreen = ({
    ...
    transfer?: InitBankTransferResponse;
}) => {
    const { data } = useLocalSearchParams();

    const parsedData = data
        ? (JSON.parse(data as string) as InitBankTransferResponse)
        : null;
        ...
        <View className="flex-1 bg-background py-6 gap-4 px-3">
            ...
            <View className="border border-border-subtle rounded-[18px] bg-input p-4 elevation">
                <View className="flex-row items-center gap-2 mb-4">
                    <CheckCircle color="gray" size={22} />
                    <Text className="font-poppins-bold text-sm text-gray-500">
                ...
                <View className="gap-3 mt-2">
                    ...
                    <View className="flex-row items-center justify-between">
                        ...
                        <View className="gap-2 flex-row">
                            <Text className="text-primary">
                                {parsedData?.transfer_account}
                            </Text>
                            <TouchableOpacity
                                onPress={() => handleCopy(parsedData?.transfer_account ?? "")}
                            >
                    ...
                    <View className="flex-row items-center justify-between">
                        ...
                        <View className="flex-row gap-2">
                            <Text className="text-primary">
                                {parsedData?.transfer_reference}
                            </Text>
                            <TouchableOpacity
                                onPress={() => handleCopy(parsedData?.transfer_reference ?? "")}
                            >
                                <Copy size={18} color="gray" />
                            </TouchableOpacity>
                        </View>
                ...
                <View className="items-center mt-4 gap-2">
                    <View className="flex-row items-center gap-2 self-start mt-5">
                        <Info size={18} color="gold" />
                        <Text className="font-poppins-bold text-yellow-400 text-xs">
                            Important
                        </Text>
                    </View>
                    <Text className="text-gray-400 text-sm">
                        Please transfer the exact amount to the above account. Use the
                        reference and note as provided. The account will expire at the
                        stated time.
                    </Text>
                </View>
